...
class VisualSimulationEnv(gym.Env):
    ...
    def step(self, action):
        self._steps += 1
        done = self.count > len(NOTIFICATION_POSITIONS)
        truncated = False
        info = {}
        ...
        # distance_dot_notification = (pow(self.dot_pos[0] - self.block_pos[0], 2) + pow(
        #     self.dot_pos[1] - self.block_pos[1], 2)) ** 0.5
        ...
        # if normalize_distance < 0.2:
        #     normalize_distance = -0.1
        ...
        if block_rect.colliderect(dot_rect):
            self.reward += 100
            self.count += 1
            if self.count < len(NOTIFICATION_POSITIONS):
                self.block_pos = NOTIFICATION_POSITIONS[self.count]
                self.dot_pos = [screen_width // 2, screen_height // 2]
