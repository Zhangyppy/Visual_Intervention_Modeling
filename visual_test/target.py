...
class Target:
    ...
    def __init__(
        ...
        if size is None:
            self.size = (
                ...
                if self.layer == LAYER_ENVIRONMENT
        ...
        self.texture = (
            GREEN_TEXTURE if self.layer == LAYER_ENVIRONMENT else BLUE_TEXTURE
        )
        self.required_steps = max(
            1, required_steps
        )  # Ensure at least 1 step is required
    ...
    def update_lifetime(self):
        """
        Update target lifetime and check if it should expire.
